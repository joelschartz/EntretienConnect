...
/*
 * EntretienConnect v330 – native macOS e-Bichelchen login window.
    ...
    EC_WEBVIEW.evaluateJavaScriptCompletionHandler($('navigator.userAgent || "Mozilla/5.0"'), function(uaValue, uaError) {
      ...
      writeState({
        ...
        session: {
          ...
          browser: 'macOS WKWebView v330'
        },
        engine: 'WKWebView-v330',
        startedAt: new Date(EC_STARTED_AT).toISOString()
      });
...
function buildControllerScript() {
  ...
  return `(() => {
    ...
    if (!window.__entretienConnectNative330) {
      window.__entretienConnectNative330 = {phase:'starting',url:href,error:'',data:null,startedAt:Date.now()};
      const s = window.__entretienConnectNative330;
      s.phase = 'reading';
      Promise.resolve(${EC_READ_EXPRESSION})
        ...
        .catch(e => {
          ...
          setTimeout(() => { try { delete window.__entretienConnectNative330; } catch (_) {} }, 1200);
        });
    }
    const s = window.__entretienConnectNative330;
