#!/usr/bin/env python3
# eBichelchenHelper v1.11.1 - lokaler Helfer für individuelle e-Bichelchen-Nachrichten.
...
# ===================================================================
# v312 – native macOS login window (WKWebView / Safari WebKit)
# -------------------------------------------------------------------
...
def _mac_wk_resolve_script() -> pathlib.Path:
    """Create the WKWebView module from the embedded v312 source every time.
    ...
    """
    target_dir = DATA_ROOT / "native-runtime" / "v312"
    target = target_dir / MAC_WK_SCRIPT_NAME
    try:
        target_dir.mkdir(parents=True, exist_ok=True)
        data = base64.b64decode(MAC_WK_SCRIPT_B64.encode("ascii"), validate=True)
        if len(data) < 1000 or b"EntretienConnect v312" not in data:
            raise RuntimeError("embedded v312 module is incomplete")
        ...
        raise RuntimeError(
            "WKWebView v312 konnte nicht vorbereitet werden (" + str(target) + "): " + str(exc)
        ) from exc
