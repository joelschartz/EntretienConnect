...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.split("?", 1)[0] == "/api/graph/capabilities":
            return self._json(200, {"ok": True, "deferredSend": True, "platform": "python", "appVersion": _helper_version(), "backendGeneration": 314, "nativeLoginEngine": ("WKWebView-v314" if sys.platform == "darwin" else "chromium-helper"), "port": getattr(self.server, "server_address", (None, PORT))[1], "ebichelchen": EB_AVAILABLE, "firefoxBidi": bool(EB_AVAILABLE and getattr(eb, "supports_firefox_bidi", lambda: False)()), "webDir": DIRECTORY, "persistDir": PERSIST_DIR})
